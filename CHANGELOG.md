...
## 📅 [2026-07-12] v1.2.0 — 智能重排与并行索引

### ✨ 新增功能

- [x] 新增 CrossEncoder reranker，使用 `cross-encoder/mmarco-mMiniLMv2-L12-H384-v1` 对混合检索候选进行二阶段重排。
- [x] `/api/chat` 新增请求级 `rerank_enabled` 参数，避免服务器全局开关影响其他用户。
- [x] Agent Loop 与一步式 RAG 均支持智能重排；模型首次启用时懒加载，失败时自动回退原混合排序。
- [x] 前端输入框新增“智能搜索”按钮，默认开启并通过 `localStorage` 持久化。
- [x] 新增 `/api/attachments/parse`，支持解析一次性聊天附件而不写入知识库。

### ⚡ 性能与可靠性

- [x] 知识库增量索引采用“并行计算、串行提交”，提升批量文档处理速度并避免向量存储并发写入。
- [x] Embeddings 缓存增加线程锁与保存快照，降低并行索引时的数据竞争风险。
- [x] 16 题离线评估中，CrossEncoder 将 Recall@5/10 从 `0.8125` 提升至 `0.9375`，MRR 从 `0.6296` 提升至 `0.9375`。

### 🧹 维护

- [x] 清理示例知识文档 `deep_learning.html`。
- [x] 测试、诊断与前端测试脚本统一迁移到本地 `tests/` 目录，并保持不进入产品提交。
- [x] README、API_REFERENCE、frontend README、前后端版本和环境变量示例同步至 `1.2.0`。

---

## 📅 [2026-06-16] v1.1.3 — 会话体验回补与批量上传

### ✅ 修复内容

- [x] 恢复前端启动时对后端状态和历史会话的重试加载。
- [x] 恢复当前会话标题展示和打开历史侧栏时刷新。
- [x] 修复拖拽上传容器导致的对话区滚动高度问题。
- [x] 过滤知识库缓存 JSON，避免 `kb_metadata` 出现在历史会话列表。

### ✨ 新增功能

- [x] 支持拖拽批量上传知识库文档。
- [x] `/api/upload` 支持多文件表单上传，并返回成功/失败明细。
- [x] 默认 `MAX_TOKENS` 调整为 `4000`。

### 📚 文档更新

- [x] README、API_REFERENCE、frontend README 和版本号统一升至 `1.1.3`。
